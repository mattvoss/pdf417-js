...
var libbcmath = module.exports = function() {};

libbcmath.PLUS = '+';
libbcmath.MINUS = '-';
libbcmath.BASE = 10;           // must be 10 (for now)
libbcmath.scale = 0;           // default scale

/**
 * Basic number structure
 */
libbcmath.bc_num = function() {
 ...
 * @return bc_num
 */
libbcmath.bc_new_num = function(length, scale) {
        var temp; // bc_num
...
libbcmath.safe_emalloc = function(size, len, extra) {
     ...
     * Create a new number
     */
libbcmath.bc_init_num = function() {
        return new libbcmath.bc_new_num(1,0);

    };

libbcmath._bc_rm_leading_zeros = function (num) {
     ...
     */
libbcmath.php_str2num = function(str) {
        var p;
        p = str.indexOf('.');
        if (p==-1) {
...
libbcmath.CH_VAL = function(c) {
        return c - '0'; //??
    };

libbcmath.BCD_CHAR = function(d) {
        return d + '0'; // ??
    };

libbcmath.isdigit = function(c) {
        return (isNaN(parseInt(c,10)) ? false : true);
    };

libbcmath.bc_str2num = function(str_in, scale) {
...
libbcmath.ODD = function(a) {
     ...
     */
libbcmath.memset = function(r, ptr, chr, len) {
        var i;
        for (i=0;i<len;i++) {
            r[ptr+i] = chr;
        }
     ...
     */
libbcmath.memcpy = function(dest, ptr, src, srcptr, len) {
        var i;
        for (i=0;i<len;i++) {
     ...
     */
libbcmath.bc_is_zero = function(num) {
        var count; // int
        var nptr; // int
...
libbcmath.bc_out_of_memory = function() {
