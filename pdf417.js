...
PDF417.prototype._intval = function(mixed_let, base) {
  ...
  // *     returns 5: 30
  let tmp;
  let type = typeof(mixed_let);

  if (type === 'boolean') {
    return +mixed_let;
  } else if (type === 'string') {
    tmp = parseInt(mixed_let, base || 10);
    return (isNaN(tmp) || !isFinite(tmp)) ? 0 : tmp;
  } else if (type === 'number' && isFinite(mixed_let)) {
    return mixed_let | 0;
  } else {
    return 0;
  }
...
PDF417.prototype._preg_split = function(pattern, subject, limit, flags) {
  ...
  // * example 3: let str = 'hypertext language programming';
  ...
  let OPTS = {};
  let optTemp = 0;
  let regexpBody = /^\/(.*)\/\w*$/.exec(pattern.toString())[1];
  let regexpFlags = /^\/.*\/(\w*)$/.exec(pattern.toString())[1];
  ...
  const _filter = function(str, strindex) {
  ...
  while (result = pattern.exec(subject)) {
    ...
    if (delim) {
      ...
      let resarr = Array.prototype.slice.call(result);
...
PDF417.prototype._array_search = function(needle, haystack, argStrict) {
  ...
  // *     example 2: let ordered_arr = array({3:'value'}, {2:'value'}, {'a':'value'}, {'b':'value'});
  // *     example 2: let key = array_search(/val/g, ordered_arr); // or let key = ordered_arr.search(/val/g);
  // *     returns 2: '3'
...
PDF417.prototype.barcode = function(code, ecl, aspectratio) {
  ...
  for(let i=0;i<sequence.length; i++) {
    let cw = this.getCompaction(sequence[i][0], sequence[i][1], true);
  ...
  let nce = (numcw + errsize + 1);
  let cols = Math.round((Math.sqrt(4761 + (68 * aspectratio * this.options.ROWHEIGHT * nce)) - 69) / 34);
  // adjust cols
  if (cols < 1) {
    cols = 1;
  } else if (cols > 30) {
  ...
  let sld = size - errsize;
  // add symbol length description
  codewords.unshift(sld);
  // calculate error correction
  let ecw = this.getErrorCorrection(codewords, ecl);
  // add error correction codewords
  codewords = codewords.concat(ecw);
  // add horizontal quiet zones to start and stop patterns
  let pstart = this._str_repeat('0', this.options.QUIETH) + this.options.start_pattern;
  let pstop = this.options.stop_pattern + "" + this._str_repeat('0', this.options.QUIETH);
  ...
  for (let r = 0; r < rows; ++r) {
    ...
    let arow = this._preg_split('//', row, -1, 'PREG_SPLIT_NO_EMPTY');
...
PDF417.prototype.getInputSequences = function(code) {
  ...
  if (numseq == null) {
    ...
    for (let n=0;n<numseq.length;n++) {
      let offset = code.indexOf(numseq[n]);
  ...
  for(let i=0; i<numseq.length; i++) {
    let seq = numseq[i];
    let seqlen = seq[0].length;
    if (seq[1] > 0) {
      // extract text sequence before the number sequence
      let prevseq = code.substr(offset, (seq[1] - offset));
      let textseq = [];
      // get text sequences
      ...
      if (textseq == null) {
        ...
        // add offset to each matched line
        for (let n=0;n<textseq.length;n++) {
          let offset = prevseq.indexOf(textseq[n]);
      ...
      for(let j=0; j<textseq.length; j++) {
        let txtseq = textseq[j];
        let txtseqlen = txtseq[0].length;
        if (txtseq[1] > 0) {
          // extract byte sequence before the text sequence
          let prevtxtseq = prevseq.substr(txtoffset, (txtseq[1] - txtoffset));
  ...
  switch(mode) {
    case 900: { // Text Compaction mode latch
      ...
      for (let i = 0; i < codelen; ++i) {
        let chval = this._ord(code.charAt(i));
        let k;
        if ((k = this._array_search(chval, this.options.textsubmodes[submode])) !== false) {
          ...
          for (let s = 0; s < 4; ++s) {
            ...
            if ((s != submode) && ((k = this._array_search(chval, this.options.textsubmodes[s])) !== false)) {
              ...
              if ((((i + 1) == codelen) || (((i + 1) < codelen)
              && (this._array_search(this._ord(code.charAt(i + 1)), this.options.textsubmodes[submode]) !== false)))
              && ((s == 3) || ((s == 0) && (submode == 1)))) {
    ...
    case 924: { // Byte Compaction mode latch
      ...
      while ((codelen = code.length) > 0) {
        ...
        if (sublen == 6) {
          console.log(bcmath);
          let t = bcmath.bcmul(''+this._ord(code.charAt(0)), '1099511627776');
...
PDF417.prototype.getErrorCorrection = function(cw, ecl) {
  ...
  let eclmaxid = (eclsize - 1);
  // initialize array of error correction codewords
  let ecw = this._array_fill(0, eclsize, 0);
  // for each data codeword
  for (let k=0; k<cw.length;k++) {
    let t1 = (cw[k] + ecw[eclmaxid]) % 929;
    for (let j = eclmaxid; j > 0; --j) {
      let t2 = (t1 * ecc[j]) % 929;
      let t3 = 929 - t2;
