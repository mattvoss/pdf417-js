var bcmath = require("./bcmath");

var PDF417 = module.exports = function() {};
...
PDF417._intval = function(mixed_var, base) {
            // http://kevin.vanzonneveld.net
            // +   original by: Kevin van Zonneveld (http://kevin.vanzonneveld.net)
            // +   improved by: stensi
...
PDF417.barcode = function(code, ecl, aspectratio) {
    ...
    var cols = Math.round((Math.sqrt(4761 + (68 * aspectratio * this.options.ROWHEIGHT * nce)) - 69) / 34);
    // adjust cols
    if (cols < 1) {
        cols = 1;
    } else if (cols > 30) {
    ...
    var pstart = this._str_repeat('0', this.options.QUIETH) + this.options.start_pattern;
    var pstop = this.options.stop_pattern + "" + this._str_repeat('0', this.options.QUIETH);
    this.options.barcode_array['num_rows'] = (rows * this.options.ROWHEIGHT) + (2 * this.options.QUIETV);
    this.options.barcode_array['num_cols'] = ((cols + 2) * 17) + 35 + (2 * this.options.QUIETH);
    this.options.barcode_array['bcode'] = [];

    var empty_row;
    // build rows for vertical quiet zone
    if (this.options.QUIETV > 0) {
        empty_row = this._array_fill(0, this.options.barcode_array['num_cols'], 0);
        for (var i = 0; i < this.options.QUIETV; ++i) {
            // add vertical quiet rows
            this.options.barcode_array['bcode'].push(empty_row);
        }
    }
    ...
    for (var r = 0; r < rows; ++r) {
        ...
        // left row indicator
        row += this._sprintf('%17b', this.options.clusters[cid][L]);
        // for each column
        for (var c = 0; c < cols; ++c) {
            row += this._sprintf('%17b', this.options.clusters[cid][codewords[k]]);
            ++k;
        }
        switch (cid) {
            case 0: {
        ...
        for (var h = 0; h < this.options.ROWHEIGHT; ++h) {
            this.options.barcode_array['bcode'].push(arow);
        }
        ++cid;
        if (cid > 2) {
            cid = 0;
        ...
        switch(mode) {
                case 900: { // Text Compaction mode latch
                        ...
                        for (var i = 0; i < codelen; ++i) {
                                ...
                                var k;
                                if ((k = this._array_search(chval, this.options.textsubmodes[submode])) !== false) {
                                        ...
                                        for (var s = 0; s < 4; ++s) {
                                                // search new sub-mode
                                                if ((s != submode) && ((k = this._array_search(chval, this.options.textsubmodes[s])) !== false)) {
                                                        // s is the new submode
                                                        if ((((i + 1) == codelen) || (((i + 1) < codelen) && (this._array_search(this._ord(code.charAt(i + 1)), this.options.textsubmodes[submode]) !== false))) && ((s == 3) || ((s == 0) && (submode == 1)))) {
                                                                ...
                                                                // latch
                                                                txtarr  = txtarr.concat(this.options.textlatch[''+submode+s]);
                                                                // set new submode
                                                                submode = s;
                                                        }
...
PDF417.getErrorCorrection = function(cw, ecl) {
        ...
        var ecc = this.options.rsfactors[ecl];
